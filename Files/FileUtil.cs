   ...
   public  class FileUtil
    {
        ...
        {
            CreateFile(filePath, text, encoding);
        ...
        {
            if (!IsExistFile(filePath)) return string.Empty; 
            //创建流读取器
        ...
        /// <param name="isLast">读取最后一行</param>
        /// <returns></returns>
        public static string[] FileTxtReadAllLines(string filePath,  bool isLast = false)
        {
            return FileTxtReadAllLines(filePath, Encoding.UTF8, isLast);
        }
        /// <summary>
        /// 将记事本文件以行的方式读取
        /// </summary>
        ...
        /// <returns></returns>
        public static string[] FileTxtReadAllLines(string filePath, Encoding encoding, bool isLast = false)
        {
            string[] resultArr = new string[] { };
            if (!IsExistFile(filePath)) return resultArr;

            try
            {
                resultArr = File.ReadAllLines(filePath);
                if (isLast && resultArr.Length > 0)
                {
                    var result = resultArr[resultArr.Length - 1];
                    resultArr = new string[] { result };
                }
